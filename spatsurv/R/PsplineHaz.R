##' PsplineHaz function
##'
##' A function to define a parametric proportional hazards model where the baseline hazard is modelled by a basis spline and 
...
PsplineHaz <- function(times,knots=quantile(times),degree=3,MLinits=NULL){

    Sys.sleep(0.2)
    warning("!!!UNDER DEVELOPMENT, YOU HAVE BEEN WARNED !!!",immediate.=TRUE)
    ...
    flist$distinfo <- function(){
        retlist <- list()
        retlist$npars <- np + 1
        retlist$parnames <- c(paste("lambda",1:np,sep=""),"tau")
        retlist$trans <- log
        retlist$itrans <- exp
        retlist$jacobian <- exp
        retlist$hessian <- lapply(1:(np+1),function(zz){return(exp)})
        if(is.null(MLinits)){
            retlist$MLinits <- rep(-10,np+1)  
        }
        else{
    ...
    flist$basehazard <- function(pars){
        fun <- function(t,...){
            ...
            if(any(is.na(idx))){
                ...
                basismatrix[basismatrix<0] <- 0 # these are small negative numbers
                return(colSums(pars[1:np]*t(basismatrix)))
            }
            else{
                return(colSums(pars[1:np]*t(basismatrix[idx,,drop=FALSE])))
            }
        }
        return(fun)  
    }
    ...
    flist$gradbasehazard <- function(pars){
        fun <- function(t,...){
            idx <- match(t,times)
            return(cbind(basismatrix[idx,],0))
        }
        return(fun)        
    }
    ...
    flist$hessbasehazard <- function(pars){
        funfun <- function(t,pars){ 
            return(matrix(0,np+1,np+1)) # note np + 1
        }
    ...
    flist$cumbasehazard <- function(pars){
        fun <- function(t,...){
            ...
            if(any(is.na(idx))){
                ...
                return(colSums(pars[1:np]*t(cbs$integral)) + cbs$toadd(pars[1:np]))
            }
            else{
                return(colSums(pars[1:np]*t(cbs$integral[idx,,drop=FALSE])) + cbs$toadd(pars[1:np]))
            }
        }
        return(fun) 
    }
    ...
    flist$gradcumbasehazard <- function(pars){
        fun <- function(t,...){
            idx <- match(t,times)
            return(cbind(cbs$integral[idx,],0))        
        }
        return(fun)    
    }
