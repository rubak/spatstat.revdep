...
sfill = function(data, where = NULL) {
  ...
  for (k in 1:ncol(data@data)) {
    ...
    data.ow = spatstat.geom::owin(range(coordinates(dpoints)[,1]), range(coordinates(dpoints)[,2]))
    data.ppp = spatstat.geom::as.ppp(coordinates(dpoints), data.ow)
    where.ow = spatstat.geom::owin(range(coordinates(where)[,1]), range(coordinates(where)[,2]))
