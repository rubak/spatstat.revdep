...
##'
##' A wrapper function of the \link[spatstat.core]{Kcross} function from the \pkg{spatstat} package (Baddeley et al. 2016) that takes epidemiological data used by \pkg{IDSpatialStats} functions and calculates the cross type K-function based on user defined case type homology
##'
...
##' @param correction type of edge correction to be applied (default set to simple 'border' edge correction). See the \link[spatstat.core]{Kcross} function in the \pkg{spatstat} package for more details
##' 
...
##'
##' A wrapper function of the \link[spatstat.core]{pcf} and \link[spatstat.core]{Kcross} functions from the \pkg{spatstat} package (Baddeley et al. 2016) that takes epidemiological data used by \pkg{IDSpatialStats} functions and calculates the cross type Pair Correlation Function based on user defined case type homology
##'
##' @param epi.data a three-column numerical matrix that contains coordinates (\code{x} and \code{y}) for each case and information on case type (e.g. genotype or serotype). First two columns must be \code{x} and \code{y}
##' @param type an integer giving the column that contains information on case type. Must be an integer or a character
##' @param hom a scalar or vector giving the homotypic case type(s). Equivalent to the 'j' point type used in the cross K function. Must be an integer or character
...
get.cross.PCF <- function(epi.data, # matrix containing xy coordinates and case type
     ...
     return(as.data.frame(spatstat.core::pcf(spatstat.core::Kcross(X=epi.data.ppp, i=0, j=1, r=r, correction=correction), spar=0.8, method='c')))
}
